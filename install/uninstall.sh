...
# Uninstalls Pi Weather Display (systemd service, timer, venv). Does not
# touch the git checkout itself.
...
SHUTDOWN_SERVICE_FILE="/etc/systemd/system/$APPNAME-shutdown.service"
...
disable_timer() {
  ...
  systemctl disable --now "$APPNAME-shutdown.service" 2>/dev/null || true
...
echo_header "The git checkout is untouched."
