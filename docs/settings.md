...
| `rain_axis_format` | `"mm"` | `"mm"` \| `"category"` - chart's rain axis for rain/hail windows: a plain mm number vs Dutch intensity words (droog/motrgn/licht/matig/zwaar/hevig). Snow/dry windows always show a plain number regardless. In `"category"` mode the rotated side label also drops its `"[mm]"` unit suffix (`"Regen [mm]"` -> `"Regen"`). In "gridlines"/"compact" screen mode, every 10° reference line on a rain/hail window also shows the rain value/word at that same height (temp and rain share one pixel-height axis). In `"category"` mode, once any of those show, the rotated side label is dropped - the wide intensity words ("motrgn" etc.) genuinely collide with it there; it still shows on a rare narrow-temp-range day where no gridline ends up labeled, and always on snow/dry windows (their gridline numbers have no self-explanatory unit/word, so those two keep the side label and skip the extra gridline values entirely). In plain `"mm"` mode the gridline's rain number is normally rounded to a whole number, no decimal - it's a geometric scale marker, not a real reading, so a decimal there doesn't usually add meaningful precision. Per explicit user ask ("set dotted lines for rain to whole numbers or 1 decimal if needed... expand the max when needed... no more rounding"), duplicate gridline values are now resolved in two layers, both in `_rain_gridline_labels`/`_choose_rain_axis_max`: (1) a colliding gridline falls back to **one** decimal place (never more, and only when `rain_axis_max` is ≤9mm - above that, always whole numbers, even if a collision remains); (2) if that alone isn't enough, `_choose_rain_axis_max` searches a handful of integer steps above the real data's natural max (`max(1, ceil(...))`) for the smallest value where every gridline comes out genuinely distinct - rather than accepting the natural max's collisions and patching them after the fact. Since `rain_axis_max` also sets the bar chart's own scale, an expanded max can leave the real data's peak short of the very top of the chart on days where expansion is needed - an accepted tradeoff, not a bug. The rain axis's own top-extreme number is handled separately: dropped outright rather than duplicating the topmost gridline (`topmost_gridline_rain_label`) - for some temp shapes (the topmost real gridline sitting very close to the day's actual high) this collision is structural and no amount of expansion avoids it, so it's deliberately excluded from the search's own pass/fail check and left to this drop instead. Even with both layers, an extreme, unrealistic-for-this-deployment temperature swing (very many gridlines) can still exhaust the search's step budget without finding a fully clean value - a known, accepted residual limit (see `docs/changes.md` entries 53-55), checked directly against all 14 of `scripts/test_locations.py`'s real diverse locations before accepting it (none reproduced it). Per explicit user ask, the rotated side label is **kept** alongside these numbers rather than dropped, pushed past the widest one actually drawn (measured per-render, not a fixed offset) so the two don't land on the same spot. |
