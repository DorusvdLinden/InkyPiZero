...
| `rain_axis_format` | `"mm"` | `"mm"` \| `"category"` - chart's rain axis for rain/hail windows: a plain mm number vs Dutch intensity words (droog/motrgn/licht/matig/zwaar/hevig). Snow/dry windows always show a plain number regardless. In `"category"` mode the rotated side label also drops its `"[mm]"` unit suffix (`"Regen [mm]"` -> `"Regen"`). In "gridlines"/"compact" screen mode, every 10° reference line on a rain/hail window also shows the rain value/word at that same height (temp and rain share one pixel-height axis). In `"category"` mode, once any of those show, the rotated side label is dropped - the wide intensity words ("motrgn" etc.) genuinely collide with it there; it still shows on a rare narrow-temp-range day where no gridline ends up labeled, and always on snow/dry windows (their gridline numbers have no self-explanatory unit/word, so those two keep the side label and skip the extra gridline values entirely). In plain `"mm"` mode the gridline's rain number is normally rounded to a whole number, no decimal - it's a geometric scale marker, not a real reading, so a decimal there doesn't usually add meaningful precision. Per explicit user ask (a sequence of real reported cases - see `docs/changes.md` entries 53-56 for the full history), the chart no longer just rounds `rain_axis_max = max(1, ceil(real max rain))` and hopes for the best - `_choose_rain_axis_max` searches a handful of integer steps above that natural value for one where `_rain_gridline_labels` reports every gridline as both **distinct** (no duplicate numbers) and **uniform** (evenly spaced gridlines show evenly *stepped* numbers, not just individually-correct-but-inconsistent ones - e.g. raw values 0/2.33/4.67/7 independently rounding to "0, 2, 5, 7" reads as wrong even though each number is accurate). A colliding gridline can still fall back to **one** decimal place (never more) as a secondary layer, but only when the chosen `rain_axis_max` is ≤`DECIMAL_ELIGIBLE_MAX_MM` (10mm) - above that, always whole numbers, even if a collision remains. Since `rain_axis_max` also sets the bar chart's own scale, an expanded max deliberately buys "headspace" above the real data's peak for a clean axis - a confirmed, explicit tradeoff, not a bug. The rain axis's own top-extreme number (what used to show `rain_axis_max` itself at the very top of the plot) is **no longer drawn at all** in this mode - per explicit user ask ("no need to show actual max at the top"), since it sits at the true pixel edge of the plot (wherever `max_temp` actually lands) rather than at the next uniform step, and could show a value that broke the clean pattern the interior gridlines just established (e.g. "8" right after "0, 3, 6"). The interior gridlines alone now carry the reading. Even with all of this, an extreme, unrealistic-for-this-deployment temperature swing (very many gridlines) can still exhaust the search's step budget without finding a fully clean value - a known, accepted residual limit, checked directly against all 14 of `scripts/test_locations.py`'s real diverse locations before accepting it (none reproduced it). Per explicit user ask, the rotated side label is **kept** alongside these numbers rather than dropped, pushed past the widest one actually drawn (measured per-render, not a fixed offset) so the two don't land on the same spot. |
