...
- [ ] **Chart temperature axis breaks for imperial/standard units**: tested via `--mock-output` 2026-08-11 (all three screen modes, both non-metric unit systems; not yet fixed). Two real bugs found in `widgets/chart.py`, both stemming from `min_temp, max_temp = min(actual_min, 0), max(actual_max, 0)` (line 69) and the fixed `grid_start`/`grid_end` step of 10 (lines 108-109) being calibrated for °C's small-magnitude, near-zero range:
  - **`standard` (Kelvin) is nearly unreadable in gridlines/compact mode**: real temps (~270-310K) always sit in the top ~10% of a 0-floored axis, and the hardcoded 10-unit gridline step produces ~30 gridlines/labels across that 0-310 span, rendering as a solid black wall of overlapping text on the left edge (`mock_display_output/units_test_standard_gridlines.png`, `units_test_standard_compact.png`). `original` mode (no gridlines) is merely uninformative rather than illegible - the plotted line is a near-flat sliver hugging the top.
  - **`imperial` (°F) wastes most of the chart's vertical space**: 0°F floor is far colder than any realistic weather range, so the plotted line hugs the top third of the chart (`units_test_imperial_original.png`). The "freezing line" dashed marker (`chart.py` ~143-147, `min_temp < 0`) is calibrated to 0°C specifically - it's simply the wrong reference point in °F (should be 32°F) and physically meaningless in K, so the feature silently never fires for either non-metric unit system.
  - **Separately, a max-value label collides with the topmost gridline tick label** in `imperial_gridlines`/`imperial_compact` ("82°F" landed right on top of the "80°F" tick, rendering as illegible overlapping glyphs, `units_test_imperial_gridlines.png`) - a general label-placement bug (no minimum-gap check between the two), just happened to be triggered by this run's specific data rather than being unit-specific itself; likely also possible in `metric` on the right data.
  - Non-chart unit conversion itself looked correct in all six renders (header temp/feels-like/min-max, wind speed mph/m/s, visibility mi/km, forecast card highs/lows, precip axis label mm/in) - the bug is localized to `widgets/chart.py`'s axis-scaling assumptions, not `weather_data.py`'s conversions.
  - Not fixed yet - pending a decision on axis-scaling approach (e.g. don't clamp the floor to a fixed 0, scale the gridline step to the data's unit/magnitude, and add minimum-gap collision handling between the max-value label and the nearest tick label).
