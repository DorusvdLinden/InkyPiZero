...
- [ ] **Buttons B/C/D GPIO pins not individually hardware-confirmed**: `button_listener.py` uses Pimoroni's standard 4-button GPIO layout (A=5, B=6, C=16, D=24) - only A was actually confirmed via a live probe during initial install; B/C/D are assumed correct by convention but have never been individually verified by pressing each one and checking the right screen mode switches. Found while searching the repo for open gaps.
...
- [x] ~~Weather condition/moon-phase/sunrise-sunset icons were individually sourced from different Flaticon authors (see attribution.md history), which is what caused the inconsistent-padding bug above in the first place - `AssetStore`'s crop-and-fit only compensated for it, didn't remove the root cause.~~ Fixed at the source: replaced with recolored PNGs rendered from [erikflowers/weather-icons](https://github.com/erikflowers/weather-icons) SVGs, a single coherent icon set with a consistent `viewBox="0 0 30 30"` per icon. See "Regenerating icons" in `docs/icons.md`.
